...
class LogSearchIndex {
  ...
  public:
   ...
   LogSearchIndex( std::vector<Kmer>& kmers, std::unique_ptr<cmph_t, Deleter>& hash, 
   ...
   LogSearchIndex( LogSearchIndex&& ph ) {
   ...
   static LogSearchIndex fromFile( const std::string& fname ) {
    ...
    LogSearchIndex index(kmers, hash, merSize, canonical);
   ...
   inline size_t index( uint64_t kmer ) {
    auto id = std::lower_bound(kmers_.begin(), kmers_.end(), kmer) - kmers_.begin();
    return (id < kmers_.size() and kmers_[id] == kmer) ? id : INVALID;
   }
