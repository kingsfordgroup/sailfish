...
class ExpresionDataset(object):
  ...
  def normalize(self, method="id"):
    if method == "id":
        ...
        mult = np.power(10,9)
        self.exps_ = [ ExpressionDatum(e.name, e.length, mult * (e.expression / (tot*e.length))) for e in self.exps_ ]
    elif method == "lrpkm":
        self.normalize("rpkm")
        self.normalize("log")
  ...
  def zipWithMatching(self, other):
    ...
    ok = lambda x: not (np.isinf(x) or np.isnan(x))
...
def parseExpressionFile(fname):
    ...
    with open(fname,'rb') as ifile:
        for l in ifile:
            ...
            dset.addElement(ExpressionDatum(dname, dlength, dexp))
